...
mkimage -A $SHDPKG_KERNEL_ARCH \
        ...
        -n 'Shedbuilt Linux 4.16rc7' \
        "${SHDPKG_BOOTPATH}/uImage" &&
mkdir -v "${SHED_FAKEROOT}/boot" &&
install -m644 System.map "${SHED_FAKEROOT}/boot/System.map-4.16rc7" &&
install -m755 "${SHDPKG_BOOTPATH}/uImage" "${SHED_FAKEROOT}/boot/linux-4.16rc7-uImage" &&
install -m644 "${SHDPKG_BOOTPATH}/dts/${SHDPKG_DTBFILE}" "${SHED_FAKEROOT}/boot/linux-4.16rc7.dtb" &&
